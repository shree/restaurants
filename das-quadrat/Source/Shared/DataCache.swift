...
class DataCache {
    ...
    private func cleanupCache() {
        privateQueue.addOperationWithBlock {
            ...
            var fileURLs = self.fileManager.contentsOfDirectoryAtURL(self.directoryURL, includingPropertiesForKeys: properties, options: NSDirectoryEnumerationOptions.SkipsHiddenFiles) as? [NSURL]
            ...
            for aFileURL in fileURLs! {
                let values = (try? aFileURL.resourceValuesForKeys(properties)) as? [String: AnyObject]
            ...
            if cacheSize > self.cacheConfiguration.maxDiskCacheSize {
                ...
                validFiles.sortInPlace {
                    ...
                    let values1 = (try? url1.resourceValuesForKeys([NSURLContentModificationDateKey])) as? [String: NSDate]
                    let values2 = (try? url2.resourceValuesForKeys([NSURLContentModificationDateKey])) as? [String: NSDate]
